...
relative_path = os.path.join("Downloads", "python_validation", "data", 
                             "PYTHON_SCRIPTS_V2", "OLS_new_results_wo_overlaps", "l8_modis")
...
output_path = os.path.join(path, 'PLOT_Landsat_8_MODIS_USA_SFSD_RLM_step_2')
...
df_cross_sat = pd.read_excel(os.path.join(path, f'USA_SFSD_Landsat_8_vs_modis_terra.xlsx'))



print(f"\n{'='*60}")
print(f"ANALYSIS FOR: {name_station}")
print(f"{'='*60}\n")
...
# ============================================================================
# DATA PREPROCESSING - filter overlapping scenes
# ============================================================================
...
if name_station in path_filters:
     path_code = path_filters[name_station]

     df_cross_sat = df_cross_sat[
         df_cross_sat['Landsat8_image_id']
         .astype(str)
...
lst_min  = df_cross_sat['Landsat8_f_LSTmean'].min()
lst_max  = df_cross_sat['Landsat8_f_LSTmean'].max()
lst_min1 = df_cross_sat['Modis_f_LSTmean'].min()
lst_max1 = df_cross_sat['Modis_f_LSTmean'].max()

delta_lst_buffer  = round(df_cross_sat['Landsat8_e_number_of_pixels'].mean())

x_all = df_cross_sat['Landsat8_Norm_LST'].to_numpy()
y_all = df_cross_sat['Modis_Norm_LST'].to_numpy()
...
print(f"Landsat LST range: [{x_all.min():.2f}, {x_all.max():.2f}]")
print(f"MODIS LST range: [{y_all.min():.2f}, {y_all.max():.2f}]")
...
def perform_normality_tests(residuals, alpha=0.05, model_name="Model"):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, f"Histogram_{model_name.replace(' ', '_')}_10_years.png"))
    plt.savefig(os.path.join(output_path, f"Histogram_{model_name.replace(' ', '_')}_10_years.pdf"))
    plt.show()
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, f"QQplot_{model_name.replace(' ', '_')}_10_years.png"))
    plt.savefig(os.path.join(output_path, f"QQplot_{model_name.replace(' ', '_')}_10_years.pdf"))
    plt.show()
    plt.close()
    ...
    print("\n" + "=" * 60)
    print("CONCLUSION:")
    if jb_pvalue > alpha and shapiro_pvalue > alpha and ks_pvalue > alpha:
...
plt.tight_layout()
plt.savefig(os.path.join(output_path, f"Huber_Weights_10_years.png"))
plt.savefig(os.path.join(output_path, f"Huber_Weights_10_years.pdf"))
plt.show()
...
equation_inliers = f"Y = {slope_inliers:.2f} X + {intercept_inliers:.2f}"
...
# Export to Excel
excel_filename = os.path.join(output_path, f"OLS_comparison_stats.xlsx")
...
axes[0].set_xlabel('Landsat 8 LST [°C]', weight='bold')
axes[0].set_ylabel('MODIS Terra LST [°C]', weight='bold')
axes[0].set_title(f'LST Comparison in {name_station} in 2013-2022 (N={len(x_all)})\nR²={metrics_ols["R²"]:.2f}, RMSE={metrics_ols["RMSE"]:.2f}', weight='bold')
...
axes[1].set_xlabel('Landsat 8 LST [°C]', weight='bold')
axes[1].set_ylabel('MODIS Terra LST [°C]', weight='bold')
axes[1].set_title(f'LST Comparison in {name_station} in 2023-2022 (N={len(x_inliers)})\nR²={metrics_inliers["R²"]:.2f}, RMSE={metrics_inliers["RMSE"]:.2f}', weight='bold')
axes[1].legend(loc='upper left')
axes[1].grid(True, alpha=0.3)
axes[1].tick_params(labelbottom=True, labelleft=True)

plt.tight_layout()
plt.savefig(os.path.join(output_path, f"2OLS_10_years.png"))
plt.savefig(os.path.join(output_path, f"2OLS_10_years.pdf"))
plt.show()


# Export final OLS (large format)
marker = 'D'  # markers = [bond 'o', drak '^', fpek '*', gcmk 's', sfsd 'D', sgp 'v', tbln 'x']

fig2 = plt.figure(figsize=(15, 15))
ax = fig2.gca()
ax.set_aspect('equal', adjustable='box')

ax.scatter(x_inliers, y_inliers, color='navy', marker=marker, s=50, label='Inliers')
ax.scatter(x_all[outliers_hub], y_all[outliers_hub], color='yellowgreen', marker=marker, s=50, label='Outliers')

ax.plot(x_inliers, y_pred_inliers, 'r--', linewidth=2, label=f'OLS: {equation_inliers}')
ax.plot(x_inliers, x_inliers, 'k-', linewidth=1, label='Y = X')
...
ax.set_ylabel(
    f'MODIS Day LST from {lst_min1:.2f} to {lst_max1:.2f} [C°] buffer 1 px',
    fontsize=22, fontweight='bold'
)
ax.set_title(
    f'LST Comparison @ {name_station} in 2013-2022 \n'
    f'(N={len(x_inliers)} inl., {outliers_hub.sum()} outl.)',
...
plt.tight_layout()
plt.savefig(os.path.join(output_path, f"OLS_{name_station}.png"))
plt.savefig(os.path.join(output_path, f"OLS_HuberInliers_10_years.pdf"))
plt.show()
