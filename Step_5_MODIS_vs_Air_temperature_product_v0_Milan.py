...
output_path = path +'NEW_Modis_Night_vs_air_temperature_'+nome_stazione+'//'
...
df_original =pd.read_csv(path+"AIR_temperature_2023//version_0//modis//original//"+nome_stazione+'_Modis_microsoft_Night_TZonly'+'.csv',sep=',')

df_downscaled = pd.read_csv(path+"AIR_temperature_2023//version_0//modis//downscaled//"+nome_stazione+'_Modis_Night_Downscaled_TZonly'+'.csv',sep=',')

df_arpa =pd.read_csv(path+"AIR_temperature_2023//arpa//"+nome_stazione+'.csv',sep=',') #sep= for ARPA ',' for Tuscany ';'
#df_arpa =pd.read_excel(path+"AIR_temperature_2023//arpa//"+nome_stazione+'.xlsx') #for Napoli
...
df_original['b_system_time_start'] = pd.to_datetime(df_original['b_system_time_start'], format='%Y-%m-%d %H:%M:%S')

# Convert 'b_system_time_start' to string format
df_original['b_system_time_start_str'] = df_original['b_system_time_start'].dt.strftime('%Y-%m-%d %H:%M:%S')

# Extract the date part
df_original['date'] = df_original['b_system_time_start'].dt.strftime('%Y-%m-%d')
...
df_arpa['Data-Ora'] = df_arpa['Data-Ora'].dt.strftime('%Y-%m-%d %H:%M:%S')
...
# Now df_arpa['Data-Ora'] contains datetime values in UTC 
...
#print('t_original\n',df_original['b_system_time_start'].head(5))
#print('t_downscaled\n',df_downscaled['date'].head(5))
#print('t_arpa\n',df_arpa['Data-Ora'].head(15))
#pdb.set_trace()
...
df_comp = df_original[['date','b_system_time_start', 'a_image_id', 'f_LSTmean','g_LSTmedian', 'j_VA']].merge(df_downscaled[['date','a_image_id_down','f_LSTmean_down','g_LSTmedian_down']], left_on='date',
                  right_on='date')
...
#Filter out anomalies from ARPA dataset or comment it
df_arpa = df_arpa[df_arpa['Medio'] != -999] 

df_comparison = df_comp[['b_system_time_start', 'a_image_id', 'f_LSTmean','g_LSTmedian', 'j_VA','a_image_id_down','date','f_LSTmean_down','g_LSTmedian_down']].merge(df_arpa[['b_system_time_start','Medio']], left_on='b_system_time_start',
...
for season in df_comparison['season'].unique():
    ...
    if len(season_data) > 0:
      ...
      plt.plot(season_data['date'], season_data['f_LSTmean'], 'bo--', label='Modis_Night')#ORIGINAL MODIS
      ...
      plt.tight_layout()
      plt.savefig(output_path+'Original_MODIS_Night_LST_vs_air_Temperature_' +nome_stazione+'_'+season+'_'+'2023'+'.png')
      plt.close()
...
for season in df_comparison['season'].unique():
    ...
    if len(season_data) > 0:         
      ...
      date_form = DateFormatter('%Y-%m')
      ax.xaxis.set_major_formatter(date_form)
      plt.plot(season_data['date'], season_data['f_LSTmean_down'], 'bo--', label='Modis_Night')#DOWNSCALED MODIS
      plt.plot(season_data['date'], season_data['Medio'], 'ro--', label="ARPA's_Weather_Station")
      plt.grid()
      plt.ylabel('Temperature [°C]',weight='bold', fontsize=25)
      plt.xlabel('Time',weight='bold', fontsize=25)
      ...
      plt.legend(loc='upper left', fontsize=22)      
      plt.tight_layout()
      plt.savefig(output_path+'Downscaled_MODIS_Night_LST_vs_air_Temperature_' +nome_stazione+'_'+season+'_'+'2023'+'.png')
      plt.close()
...
date_form = DateFormatter('%Y-%m')
ax.xaxis.set_major_formatter(date_form)
plt.plot(df_comparison['date'], df_comparison['f_LSTmean'], 'bo--', label='Modis_Night_Original')#ORIGINAL MODIS
plt.plot(df_comparison['date'], df_comparison['f_LSTmean_down'], 'go--', label='Modis_Night_Downscaled')
plt.plot(df_comparison['date'], df_comparison['Medio'], 'ro--', label="ARPA's_weather_station")
plt.grid()
plt.ylabel('Temperature [°C]', weight='bold', fontsize=25)
plt.xlabel('Time', weight='bold', fontsize=25)
# Adjusting tick parameters for larger fontsize
...
plt.legend(loc='upper left', fontsize=22)
plt.tight_layout()
plt.savefig(output_path+'NEW_V0_MODIS_Night_LST_vs_air_Temperature_' +nome_stazione+'_2023'+'.png')
plt.savefig(output_path+'NEW_V0_MODIS_Night_LST_vs_air_Temperature_' +nome_stazione+'_2023'+'.pdf')
    
    
# fig, ax = plt.subplots(figsize=(20, 15))
# plt.title('Temporal Relationship Between MODIS Night LST vs Air Temperature @' + nome_stazione + '_2023',weight='bold', fontsize=25) 
...
# plt.plot(df_comparison['date'], df_comparison['f_LSTmean'] + offset, '-', label='Modis_Night_Original', color='blue')
# plt.plot(df_comparison['date'], df_comparison['f_LSTmean_down'], '-', label='Modis_Night_Downscaled', color='green')
# plt.plot(df_comparison['date'], df_comparison['Medio']- offset, '-', label="ARPA's_weather_station", color='red')
# plt.grid()
...
# plt.yticks([])
# plt.tight_layout()
# plt.savefig(output_path + 'V0_MODIS_Night_LST_TREND_vs_air_Temperature_' + nome_stazione + '_' + '2023' + '.png')
...
excel_filename = output_path + 'new_v0_stats_Modis_vs_Milan_' + nome_stazione + '.xlsx'
