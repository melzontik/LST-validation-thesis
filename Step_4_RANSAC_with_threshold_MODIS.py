...
output_path = path +'MODIS_night_Naples_ransac'+'//'
...
df_super = reading_file(path+'per_pixel_data//modis_v1//naples//super_resolved//'+"super_resolved_2023-01-01"+".csv")
df_orig = reading_file(path+'per_pixel_data//modis_v1//naples//original//'+"original_2023-01-01"+".csv")
...
x_for_RANSAC = df_orig['median'].to_numpy() #change if needed
y_for_RANSAC = df_super['median'].to_numpy() #control the values in the columns of both files!
...
#ax = fig.gca()
plt.title('Naples: LST comparison original MODIS night-time\n VS MODIS downscaled '+ df_super['image_date'].iloc[0],weight='bold', fontsize=25)
...
df_export.to_excel(output_path + 'naples_ransac_' + date + '.xlsx', index=False)