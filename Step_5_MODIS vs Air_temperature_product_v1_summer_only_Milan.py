...
nome_stazione = 'Brera'
output_path = path +'MODIS_Night_vs_ARPA_v1_new_'+nome_stazione+'//'
...
df_original =pd.read_csv(path+"AIR_temperature_2024//version_1//modis//original//"+nome_stazione+'_Modis_microsoft_Night_TZonly'+'.csv',sep=',')

df_downscaled = pd.read_csv(path+"AIR_temperature_2024//version_1//modis//downscaled//"+nome_stazione+'_Modis_Night_Downscaled_TZonly'+'.csv',sep=',')

df_gee = pd.read_csv(path+"AIR_temperature_2024//version_1//modis//gee_modis//"+nome_stazione+'_Modis_Night_Aqua_Terra_buffer_01'+'.csv',sep=',')

df_arpa =pd.read_csv(path+"AIR_temperature_2024//arpa//"+nome_stazione+'.csv',sep=',') #for ARPA ',' for Tuscany ';'
#df_arpa =pd.read_excel(path+"AIR_temperature_2024//arpa//"+nome_stazione+'.xlsx') #for Naples

#df_original = df_original.dropna() #comment it for station Giusto and Casal Nuovo
...
#ROUND ARPA 10 min, Tuscany 15 min, Napoli 1h
df_gee['b_system_time_start'] = pd.to_datetime(df_gee['b_system_time_start'], format='%Y-%m-%d %H:%M:%S').dt.round('10min')


# ARPA original format UTC+1 hour. GEE in Local solar time and in UTC. choose the right column!
...
#REMEMBER! 
# MODIS local solar time (NO NEED to convert in UTC the weather data)
# Landsat UTC (convert the weather datasets in UTC!)
# ARPA local solar time (i valori ottenuti fino ad ora indicata)
# Tuscany local solar time without winter/summer changes European central time
# Campania local solar time
# GBOV UTC


# # Define the timezone for the input datetime (ORA SOLARE - UTC+1)
# input_timezone = pytz.timezone('Europe/Rome')

# # Localize the datetime column to the input timezone
# df_arpa['Data-Ora'] = df_arpa['Data-Ora'].dt.tz_localize(input_timezone)

# # Define the target timezone (UTC)
# output_timezone = pytz.utc

# # Convert the datetime column to the target timezone
# df_arpa['Data-Ora'] = df_arpa['Data-Ora'].dt.tz_convert(output_timezone)

# # If you want to remove the timezone information and have a naive datetime column
...
# Perform the merge
df_gee_arpa = df_gee[['b_system_time_start', 'c_date', 'g_LSTmedian']].merge(
...
df_gee_arpa['c_date'] = df_gee_arpa['c_date'].str.strip()
df_gee_arpa['c_date'] = pd.to_datetime(df_gee_arpa['c_date'], format='%Y-%m-%d')

# Extract year and month for grouping
df_gee_arpa['year_month'] = df_gee_arpa['c_date'].dt.to_period('M')
...
df_gee_arpa['i_local_solar_time_seconds'] = (
...
monthly_medians = df_gee_arpa.groupby('year_month').agg({
    'Medio' : 'median',
    'i_local_solar_time_seconds': 'median'
}).reset_index()
...
# Convert 'i_local_solar_time_seconds' back to time format
monthly_medians['i_local_solar_time_median'] = pd.to_timedelta(
    monthly_medians['i_local_solar_time_seconds'], unit='s'
).apply(lambda x: (x.components.hours, x.components.minutes, x.components.seconds))

# Format time as "HH:MM:SS"
monthly_medians['i_local_solar_time_median'] = monthly_medians['i_local_solar_time_median'].apply(
    lambda x: f"{x[0]:02}:{x[1]:02}:{x[2]:02}"
)

# Select and rename columns for clarity
monthly_medians = monthly_medians[['month_start_date', 'Medio', 'i_local_solar_time_median']]
...
df_comp = df_gee_arpa[['month_start_date', 'Medio']].merge(df_downscaled[['b_system_time_start', 'g_LSTmedian_down']],left_on='month_start_date',right_on='b_system_time_start', how='inner')
...
# Convert 'b_system_time_start' to datetime format
#df_original['b_system_time_start'] = df_original['b_system_time_start'].dt.strftime('%Y-%m-%d')
df_original['b_system_time_start'] = pd.to_datetime(df_original['b_system_time_start']).dt.normalize()
df_comp['b_system_time_start'] = pd.to_datetime(df_comp['b_system_time_start']).dt.normalize()

df_final = df_comp[['b_system_time_start','month_start_date','g_LSTmedian_down','Medio']].merge(df_original[['b_system_time_start', 'g_LSTmedian']],left_on='b_system_time_start',right_on='b_system_time_start')
...
seasons = [('winter', (date(Y,  1,  1),  date(Y,  2, 28))),
           ...
           ('summer', (date(Y,  6, 1),  date(Y,  9, 30))),#all 4 months as summer
           ('autumn', (date(Y,  10, 1),  date(Y, 11, 30))),
...
for season in df_final['season'].unique():
    ...
    if len(season_data) > 0:
      ...
      plt.title('Original MODIS Night LST vs Air Temperature \n@' + nome_stazione + '_' + season + '_' + '2024',weight='bold', fontsize=25)
      ax = fig.gca()
      ax.xaxis.set_major_locator(mdates.MonthLocator())
      date_form = DateFormatter('%Y-%m')
      ax.xaxis.set_major_formatter(date_form)
      plt.plot(season_data['month_start_date'], season_data['g_LSTmedian'], 'bo--', label='MODIS Night')#ORIGINAL MODIS
      plt.plot(season_data['month_start_date'], season_data['Medio'], 'ro--', label="ARPA's_Weather_Station")
      ...
      plt.grid()      
      #plt.tight_layout()
      plt.savefig(output_path+'V1_Original_MODIS_LST_vs_air_Temperature_' +nome_stazione+'_'+season+'_'+'2024'+'.png')
      plt.close()
...
for season in df_final['season'].unique():
    ...
    if len(season_data) > 0:         
      ...
      fig = plt.figure(figsize=(15, 15))
      plt.title('Downscaled MODIS Night LST vs Air Temperature \n@' + nome_stazione + '_' + season + '_' + '2024',weight='bold', fontsize=25)
      ax = fig.gca()
      ax.xaxis.set_major_locator(mdates.MonthLocator())
      date_form = DateFormatter('%Y-%m')
      ax.xaxis.set_major_formatter(date_form)
      plt.plot(season_data['month_start_date'], season_data['g_LSTmedian_down'], 'bo--', label='MODIS Night')#DOWNSCALED MODIS
      plt.plot(season_data['month_start_date'], season_data['Medio'], 'ro--', label="ARPA's_Weather_Station")
      plt.grid()
      plt.ylabel('Temperature [°C]',weight='bold', fontsize=25)
      plt.xlabel('Time',weight='bold', fontsize=25)
      ...
      plt.legend(loc='lower center', fontsize=20)      
      #plt.tight_layout()
      plt.savefig(output_path+'V1_Downscaled_Modis_LST_vs_air_Temperature_' +nome_stazione+'_'+season+'_'+'2024'+'.png')
      plt.close()
...
fig = plt.figure(figsize=(15, 15))
plt.title('MODIS Night LST vs Air Temperature in Milan \n@' + nome_stazione + '_2024',weight='bold', fontsize=25)
ax = fig.gca()
ax.xaxis.set_major_locator(mdates.MonthLocator())
date_form = DateFormatter('%Y-%m')
ax.xaxis.set_major_formatter(date_form)
plt.plot(df_final['month_start_date'], df_final['g_LSTmedian'], 'bo--', label='MODIS_Night_Original')#ORIGINAL Landsat
plt.plot(df_final['month_start_date'], df_final['g_LSTmedian_down'], 'go--', label='MODIS_Night_Downscaled')
plt.plot(df_final['month_start_date'], df_final['Medio'], 'ro--', label="ARPA's_weather_station")
plt.grid()
plt.ylabel('Temperature [°C]',weight='bold', fontsize=25)
plt.xlabel('Time',weight='bold', fontsize=25)
plt.legend(loc='lower center', fontsize=20)
...
ax.tick_params(axis='both', which='major', labelsize=20)
plt.tight_layout()
plt.savefig(output_path+'MODIS_Night_LST_vs_air_Temperature_' +nome_stazione+'_2024_v1'+'.png')
    
    
fig, ax = plt.subplots(figsize=(15, 15))
plt.title('Temporal Relationship Between MODIS Night LST \nvs Air Temperature @' + nome_stazione + '_2024',weight='bold', fontsize=25) 
...
plt.plot(df_final['month_start_date'], df_final['g_LSTmedian'] + offset, '-', label='MODIS_Night_Original', color='blue')
plt.plot(df_final['month_start_date'], df_final['g_LSTmedian_down'], '-', label='MODIS_Night_Downscaled', color='green')
plt.plot(df_final['month_start_date'], df_final['Medio']- offset, '-', label="ARPA's_weather_station", color='red')
plt.grid()
plt.xlabel('Time',weight='bold', fontsize=25)
...
plt.savefig(output_path + 'TREND_MODIS_Night_LST_vs_air_Temperature_' + nome_stazione + '_' + '2024_v1' + '.png')
...
excel_filename = output_path + 'stats_MODIS_vs_ARPA_' + nome_stazione + '.xlsx'
